    ...
    <div className="min-h-screen bg-sky-900 text-sky-200 flex flex-col items-center py-12 px-4">
      ...
      <main className="w-full max-w-2xl bg-sky-800 rounded-xl shadow-sm border border-sky-700 p-8">
        <form onSubmit={handleSubmit} className="space-y-4">
          <label htmlFor="url-input" className="block text-sm font-medium text-sky-100">Enter URL to scan</label>
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-2.5 border border-sky-700 rounded-lg bg-sky-950 text-sky-100 placeholder:text-sky-300 focus:ring-2 focus:ring-sky-400 focus:border-sky-400 outline-none transition"
            />
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-sky-500 text-white font-medium rounded-lg hover:bg-sky-600 focus:ring-4 focus:ring-sky-200 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {result && (
          <div className="mt-8 pt-6 border-t border-sky-700">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-sky-100">Analysis Result</h2>
              <span className={`px-3 py-1 text-sm font-bold rounded-full border ${getScoreColor(result.score)}`}>
                Score: {result.score}/5
              </span>
            </div>
            <div className="bg-sky-950 p-4 rounded-lg border border-sky-700">
              <p className="text-sm font-medium text-sky-200 mb-1">Risk Level</p>
              <p className="text-lg font-bold text-sky-100 mb-3">{result.risk_level}</p>
              <p className="text-sm font-medium text-sky-200 mb-1">Explanation</p>
              <p className="text-sky-100 leading-relaxed">{result.explanation}</p>
            </div>
          </div>
        )}
      </main>
